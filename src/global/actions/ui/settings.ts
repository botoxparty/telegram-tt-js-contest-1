...
addCallback((global: GlobalState) => {
  ...
  if (settings.theme !== prevSettings.theme) {
    const withAnimation = document.hasFocus()
  ...
  if (settings.messageTextSize !== prevSettings.messageTextSize) {
    document.documentElement.style.setProperty(
      '--composer-text-size',
      `${Math.max(settings.messageTextSize, IS_IOS ? 16 : 15)}px`,
    );
    document.documentElement.style.setProperty(
      '--message-meta-height',
    ...
    document.documentElement.style.setProperty(
      '--message-text-size',
      `${settings.messageTextSize}px`,
    );
    document.documentElement.setAttribute(
      'data-message-text-size',
      settings.messageTextSize.toString(),
    );
  }
  ...
  if (
    ...
    callApi(
      ...
      Boolean(settings.shouldForceHttpTransport),
    );
  }

  if (
    settings.shouldAllowHttpTransport !== prevSettings.shouldAllowHttpTransport
  ) {
    callApi(
      'setAllowHttpTransport',
      Boolean(settings.shouldAllowHttpTransport),
    );
    if (
      !settings.shouldAllowHttpTransport
      && settings.shouldForceHttpTransport
    ) {
      global = getGlobal();
      global = {
        ...global,
  ...
  if (
    settings.shouldDebugExportedSenders
    !== prevSettings.shouldDebugExportedSenders
  ) {
    updateShouldDebugExportedSenders();
  }
...
addActionHandler(
  'updateShouldDebugExportedSenders',
  (global): ActionReturnType => {
    const { settings } = global;
    callApi(
      'setShouldDebugExportedSenders',
      Boolean(settings.byKey.shouldDebugExportedSenders),
    );
  },
);
...
addActionHandler('setChatFolderLayout', (global, actions, payload): ActionReturnType => {
  const { layout } = payload;

  // Update global state with new layout setting
  global = replaceSettings(global, { chatFolderLayout: layout });
